...
class GrinderBLETool:
    ...
    def _parse_single_file_data(self, file_data: bytes, session_id: int) -> Tuple[List[Dict], List[Dict], List[Dict]]:
        ...
        [GrindEvent x event_count (46 bytes each)]
        [GrindMeasurement x measurement_count (22 bytes each)]
        """
        if len(file_data) < 120:  # 20-byte header + 100-byte GrindSession
        ...
        # Parse 20-byte TimeSeriesSessionHeader
        ...
        #   uint32_t checksum;
        # }
        hdr_session_id, event_count, measurement_count, hdr_session_ts, hdr_session_size, hdr_checksum = \
            struct.unpack_from('<IHHIII', file_data, offset)
        offset += 20

        if hdr_session_id != session_id:
            raise ValueError(f"Header session ID mismatch: expected {session_id}, got {hdr_session_id}")
        
        # Extract full GrindSession struct at exact offsets (100-byte struct)
        session_struct_size = 100
        session_bytes = file_data[offset:offset + session_struct_size]
        
        # Extract fields at their exact offsets from debug output
        parsed_session_id = struct.unpack_from('<I', session_bytes, 0)[0]  # offset 0
        session_timestamp = struct.unpack_from('<I', session_bytes, 4)[0]  # offset 4
        profile_id = struct.unpack_from('<B', session_bytes, 8)[0]  # offset 8
        target_weight = struct.unpack_from('<f', session_bytes, 12)[0]  # offset 12
        tolerance = struct.unpack_from('<f', session_bytes, 16)[0]  # offset 16
        final_weight = struct.unpack_from('<f', session_bytes, 64)[0]  # offset 64
        latency_to_coast_ratio = struct.unpack_from('<f', session_bytes, 28)[0] # offset 28
        error_grams = struct.unpack_from('<f', session_bytes, 68)[0]  # offset 68
        total_time_ms = struct.unpack_from('<I', session_bytes, 72)[0]  # offset 72
        pulse_count = struct.unpack_from('<B', session_bytes, 76)[0]  # offset 76
        result_bytes = session_bytes[77:93]  # offset 77, 16 bytes for result_status
        total_motor_on_time_ms = struct.unpack_from('<I', session_bytes, 96)[0]  # offset 96
        ...
        session = {
            'session_id': parsed_session_id, 'session_timestamp': session_timestamp, 'profile_id': profile_id,
            'target_weight': target_weight, 'tolerance': tolerance, 'final_weight': final_weight,
            'error_grams': error_grams, 'total_time_ms': total_time_ms, 'pulse_count': pulse_count, 'latency_to_coast_ratio': latency_to_coast_ratio,
            'result_status': result_status, 'total_motor_on_time_ms': total_motor_on_time_ms
        }
        
        events = []
        # Unpack Events - 46 bytes each
        EVENT_SIZE = 46
        ...
        for event_idx in range(event_count):
            ...
            phase_id = struct.unpack_from('<B', event_bytes, 4)[0]
            pulse_attempt_number = struct.unpack_from('<B', event_bytes, 5)[0]
            event_sequence_id = struct.unpack_from('<H', event_bytes, 6)[0]
            duration_ms = struct.unpack_from('<I', event_bytes, 8)[0]
            start_weight = struct.unpack_from('<f', event_bytes, 12)[0]
            end_weight = struct.unpack_from('<f', event_bytes, 16)[0]
            motor_stop_target_weight = struct.unpack_from('<f', event_bytes, 20)[0]
            pulse_duration_ms = struct.unpack_from('<f', event_bytes, 24)[0]
            grind_latency_ms = struct.unpack_from('<I', event_bytes, 28)[0]
            settling_duration_ms = struct.unpack_from('<I', event_bytes, 32)[0]
            pulse_flow_rate = struct.unpack_from('<f', event_bytes, 36)[0]
            loop_count = struct.unpack_from('<H', event_bytes, 40)[0]
            
            # VALIDATION 2a: Verify session ID in event matches file session ID
            if timestamp_ms == 0xFFFFFFFF or phase_id == 0xFF:  # Skip invalid/empty events
                expected_event_sequence += 1
                offset += EVENT_SIZE
                continue
            
            # VALIDATION 2b: Verify event sequence ID is in order
            if event_sequence_id != expected_event_sequence:
                raise ValueError(f"Event sequence out of order: expected {expected_event_sequence}, got {event_sequence_id} at event {event_idx}")
        ...
        measurements = []
        # Unpack Measurements - 22 bytes each
        MEASUREMENT_SIZE = 22
        ...
        for meas_idx in range(measurement_count):
            ...
            motor_is_on = struct.unpack_from('<B', meas_bytes, 16)[0]
            phase_id = struct.unpack_from('<B', meas_bytes, 17)[0]
            motor_stop_target_weight = struct.unpack_from('<f', meas_bytes, 18)[0]
            
            # VALIDATION 3a: Skip invalid/empty measurements
            ...
            # VALIDATION 3b: Verify measurement sequence is in order
            # Note: We use expected_measurement_sequence as the sequence_id since measurements 
            # should be sequential from 0
            actual_sequence_id = expected_measurement_sequence
            
            offset += MEASUREMENT_SIZE
            measurement = {
                'session_id': parsed_session_id, 'sequence_id': actual_sequence_id, 'timestamp_ms': timestamp_ms,
                'weight_grams': weight_grams, 'weight_delta': weight_delta,
                'flow_rate_g_per_s': flow_rate_g_per_s, 'motor_is_on': motor_is_on,
                'phase_id': phase_id, 'phase_name': PHASE_NAMES.get(phase_id, 'UNKNOWN'),
                'motor_stop_target_weight': motor_stop_target_weight
            }
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            cursor.execute("""
                CREATE TABLE grind_sessions (
                    session_id INTEGER PRIMARY KEY, session_timestamp INTEGER,
                    profile_id INTEGER, target_weight REAL, tolerance REAL,
                    final_weight REAL, error_grams REAL, total_time_ms INTEGER, pulse_count INTEGER, latency_to_coast_ratio REAL,
                    result_status TEXT, total_motor_on_time_ms INTEGER,
            ...
            # Insert data
            cursor.executemany("INSERT INTO grind_sessions (session_id, session_timestamp, profile_id, target_weight, tolerance, final_weight, error_grams, total_time_ms, pulse_count, latency_to_coast_ratio, result_status, total_motor_on_time_ms) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                [(s['session_id'], s['session_timestamp'], s['profile_id'], s['target_weight'], s['tolerance'], s['final_weight'], s['error_grams'], s['total_time_ms'], s['pulse_count'], s.get('latency_to_coast_ratio', 1.0), s['result_status'], s['total_motor_on_time_ms']) for s in sessions])
            ...
            cursor.executemany("INSERT INTO grind_events VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                [(e['session_id'], e['event_sequence_id'], e['timestamp_ms'], e['phase_id'], 
                  ...
                  e['pulse_flow_rate'], e['loop_count']) for e in events])
