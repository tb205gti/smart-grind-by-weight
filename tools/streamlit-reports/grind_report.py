...
sessions_df['display_name'] = sessions_df.apply(
    lambda row: f"#{row['session_id']} - {row['profile_name']} - {row['target_weight']:.1f}g ({row['error_grams']:+.2f}g)", axis=1
)
...
if analysis_mode == "Single Session":
    ...
    with tab1:
        ...
        # "normal" = red for negative, green for positive.
        # "inverse" = green for negative, red for positive.
        is_within_tolerance = abs(session_data['error_grams']) < TOLERANCE_G
        is_negative = session_data['error_grams'] < 0
        # If within tolerance, we want green. If not, we want red.
