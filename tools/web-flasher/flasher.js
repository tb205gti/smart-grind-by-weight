...
function buildAssetInfo(asset) {
    ...
    const proxyUrl = isSameOrigin(directUrl) ? directUrl : `${CORS_PROXY_BASE}${encodeURIComponent(directUrl)}`;
...
async function downloadFirmware(url) {
    ...
    try {
        ...
        if (url.startsWith('http') && !url.startsWith(window.location.origin)) {
            ...
            fetchUrl = `${CORS_PROXY_BASE}${encodeURIComponent(url)}`;
...
function updateManifestFirmware() {
    ...
    const selectedOption = select.selectedOptions[0];
    const manifestUrl = selectedOption?.dataset?.manifestProxy || selectedOption?.dataset?.manifest || select.value;
    ...
    try {
        ...
        normalized.forEach(release => {
            ...
            if (release.manifest && (!release.prerelease || showRC)) {
                ...
                option.value = release.manifest.directUrl;
                option.textContent = label;
                option.dataset.display = label;
                option.dataset.version = release.version;
                option.dataset.releaseTag = release.tag;
                option.dataset.manifest = release.manifest.directUrl;
                option.dataset.manifestProxy = release.manifest.proxyUrl;
                option.dataset.manifestApi = release.manifest.apiUrl;
                option.dataset.firmware = release.firmware.directUrl;
                option.dataset.firmwareProxy = release.firmware.proxyUrl;
            ...
            if (release.ota && (!release.prerelease || showRCOTA)) {
                const option = document.createElement('option');
                option.value = release.ota.directUrl;
                option.textContent = label;
                option.dataset.display = label;
                option.dataset.version = release.version;
                option.dataset.releaseTag = release.tag;
                option.dataset.ota = release.ota.directUrl;
                option.dataset.otaProxy = release.ota.proxyUrl;
                option.dataset.firmware = release.firmware.directUrl;
                option.dataset.firmwareProxy = release.firmware.proxyUrl;
                option.dataset.prerelease = release.prerelease ? 'true' : 'false';
